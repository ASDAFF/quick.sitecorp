...
<div class="element" data-element_id="<?=$arResult['ID']?>" data-page="detail">
    <div class="card-top clearfix">
        <div class="pull-left topTabs">
            <a href="#tabs-1"><span><?=GetMessage('QUICK_CATALOG_ELEMENT_DESCRIPTION')?></span></a>
            <?if (!empty($arResult['PROPERTIES']['DOCS']['VALUE'])){?>
                <a href="#tabs-2"><span><?=GetMessage('QUICK_CATALOG_ELEMENT_DOCS')?></span></a>
            <?}?>
            <a href="#tabs-3"><span><?=GetMessage('QUICK_CATALOG_ELEMENT_HOW_BUY')?></span></a>
            <a href="#tabs-4"><span><?=GetMessage('QUICK_CATALOG_ELEMENT_SHIPPING')?></span></a>
            <a href="#tabs-5"><span><?=GetMessage('QUICK_CATALOG_ELEMENT_PAYMENT')?></span></a>
            <a href="#tabs-6"><span><?=GetMessage('QUICK_CATALOG_ELEMENT_WARRANTY')?></span></a>
            <a href="#tabs-7"><span><?=GetMessage('QUICK_CATALOG_ELEMENT_RETURN')?></span></a>
            <?if ($arParams['QUICK_ALLOW_REVIEWS'] == 'Y'){?>
                <a href="#tabs-8"><span><?=GetMessage('QUICK_CATALOG_ELEMENT_REVIEWS')?></span> <i>(<?=($arResult['BLOG_COMMENTS_CNT']['VALUE']) ? $arResult['BLOG_COMMENTS_CNT']['VALUE'] : 0?>)</i></a>
            <?}?>
        </div>
        <?/*<div class="pull-right">
            <a href="#"><?=GetMessage('QUICK_CATALOG_ELEMENT_FOUND_ERROR')?></a>
            <a href="#"><?=GetMessage('QUICK_CATALOG_ELEMENT_SEND_DESCRIPTION_EMAIL')?></a>
            <a href="#" class="btn-print"><?=GetMessage('QUICK_CATALOG_ELEMENT_PRINT')?></a>
        </div>*/?>
    </div>
            ...
            <?if (!empty($arResult['DISPLAY_PROPERTIES'])){?>
                <div>
                    <div class="characteristics">
                        ...
                        <?}?>
                        <div class="topTabs"><a href="#tabs-1" class="btn-style5 toogle-block-title"><span><?=GetMessage('QUICK_CATALOG_ELEMENT_DETAILED_FEATURES')?></span></a></div>
                    </div>
                </div>
            <?}?>
...
$arPrice = QUICKGetPrice(
                    ...
                    <div class="card-price card-price-sale">
                        <div class="col1">
                            <?if (!empty($arPrice['OLD_PRICE'])){?>
                                <p><?=GetMessage('QUICK_CATALOG_ELEMENT_OLD_PRICE')?>:</p>
                                <div class="price-old old_price"><?=$arPrice['OLD_PRICE']?></div>
                                <br/>
                                <p><?=GetMessage('QUICK_CATALOG_ELEMENT_NEW_PRICE')?>:</p>
                            <?} else {?>
                                <p><?=GetMessage('QUICK_CATALOG_ELEMENT_PRICE')?>:</p>
                            <?}?>
                            <div class="price"><?=$arPrice['PRICE']?></div>
                            <?if (!empty($arPrice['OLD_PRICE'])){?>
                                <br/><br/>
                                <div class="price_diff_box"><?=GetMessage('QUICK_CATALOG_ELEMENT_SALE')?>: <span class="price_diff"><?=$arPrice['PRICE_DIFF']?></span></div>
                            <?}?>
                        </div>
                    </div>
                <?}?>
                <div class="lvl2">
                    <?if ($arResult['PROPERTIES']['QUICK_ORDER']['VALUE'] == 'Y'){?>
                        <a class="btn-style4 btn-fast-order popbutton" data-window="order_product"><?=GetMessage('QUICK_CATALOG_ELEMENT_QUICK_ORDER')?></a>
                    <?}?>
                    <?if ($arResult['PROPERTIES']['QUESTION_PRODUCT']['VALUE'] == 'Y'){?>
                        <a class="btn question_product popbutton" data-window="question_product"><?=GetMessage('QUICK_CATALOG_ELEMENT_ASK_QUESTION')?></a>
                    <?}?>
                </div>
            </div>
    ...
    <div id="tabs">
        <div class="tabs">
            <ul>
                <li class="current"><a href="#tabs-1"><span><?=GetMessage('QUICK_CATALOG_ELEMENT_DESCRIPTION')?></span></a></li>
                <?if (!empty($arResult['PROPERTIES']['DOCS']['VALUE'])){?>
                    <li><a href="#tabs-2"><span><?=$arResult['PROPERTIES']['DOCS']['NAME']?></span></a></li>
                <?}?>
                <li><a href="#tabs-3"><span><?=GetMessage('QUICK_CATALOG_ELEMENT_HOW_BUY')?></span></a></li>
                <li><a href="#tabs-4"><span><?=GetMessage('QUICK_CATALOG_ELEMENT_SHIPPING')?></span></a></li>
                <li><a href="#tabs-5"><span><?=GetMessage('QUICK_CATALOG_ELEMENT_PAYMENT')?></span></a></li>
                <li><a href="#tabs-6"><span><?=GetMessage('QUICK_CATALOG_ELEMENT_WARRANTY')?></span></a></li>
                <li><a href="#tabs-7"><span><?=GetMessage('QUICK_CATALOG_ELEMENT_RETURN')?></span></a></li>
                <?if (!$arParams['QUICK_QUICK_VIEW'] && $arParams['QUICK_ALLOW_REVIEWS'] == 'Y'){?>
                    <li><a href="#tabs-8"><span><?=GetMessage('QUICK_CATALOG_ELEMENT_REVIEWS')?></span> <i>(<?=($arResult['BLOG_COMMENTS_CNT']['VALUE']) ? $arResult['BLOG_COMMENTS_CNT']['VALUE'] : 0?>)</i></a></li>
                <?}?>
            </ul>
        </div>
        ...
        <div class="tab" id="tabs-1">
            ...
            <?if (!empty($arResult['DETAIL_TEXT'])){?>
                <div>
                    <div class="title-style3"><?=GetMessage('QUICK_CATALOG_ELEMENT_DESCRIPTION')?></div>
                    <div class="text2">
                        ...
                        <?foreach ($arResult['PROPERTIES']['DOCS']['VALUE'] as $docs) {
                            ...
                            <div class="in-row-bot">
                                ...
                                <div>
                                    ...
                                    <div><?=GetMessage('QUICK_CATALOG_ELEMENT_SIZE')?>: <?=round($doc['FILE_SIZE'] / 1024, 1)?> <?=GetMessage('QUICK_CATALOG_ELEMENT_KB')?></div>
                                </div>
                            </div>
                        <?}?>
                    </div>
