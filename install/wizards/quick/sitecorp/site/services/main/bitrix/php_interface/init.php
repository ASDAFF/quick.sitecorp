...
function QUICKGetPrice($price = '', $saleValue = '', $saleType = '', $currency = ''){
    $arPrice = array();
    if ($saleValue > 0) {
...
AddEventHandler("iblock", "OnAfterIBlockElementUpdate", Array("QUICKQuestions", "ResponseUser"));

class QUICKQuestions
{
    function ResponseUser(&$arFields)
    {
        if ($arFields["IBLOCK_ID"] == "#QUICK_IBLOCK_ID_QUESTIONS_ANSWER#") {
            $arFilter = Array("IBLOCK_ID" => "#QUICK_IBLOCK_ID_QUESTIONS_ANSWER#", "ID" => $arFields['ID'], "PROPERTY_SEND_MAIL_VALUE" => "Y");
            ...
            if ($ob = $res->GetNextElement()) {
                ...
                if ($data['PROPERTY_QUESTION_REPLY_VALUE']['TEXT'] != '') {
                    ...
                    CEvent::Send("QUICK_NEW_QUESTION_REPLY", "#SITE_ID#", $arEventFields);
                }
            }
        }
    }
